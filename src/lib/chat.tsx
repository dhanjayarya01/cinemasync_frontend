...
export default function Chat({
  ...
  //  Live voice streaming with proper audio chunks
  ...
  //  Quick voice mode toggle with debugging
  ...
  //  Listen for incoming live voice streams
  useEffect(() => {
    if (socketManager && socketManager.socket) {
  ...
  //  live voice playback with audio context for smoother streaming
  const audioContextRef = useRef<AudioContext | null>(null);
  const audioBufferQueueRef = useRef<AudioBuffer[]>([]);
  const isPlayingLiveAudioRef = useRef<boolean>(false);

  // Voice Activity Detection to prevent sending silent audio
    ...
    <>
      ...
      <div className="absolute top-3 right-3 z-50 flex items-center space-x-2 pointer-events-auto">
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all duration-300 shadow-lg ${isLiveVoiceMode
        ...
        <button
          ...
          className="inline-flex items-center justify-center p-2 rounded-full bg-purple-600/90 text-white shadow-md hover:bg-purple-500 focus:outline-none relative"
        >
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${voiceChatAutoPlay ? 'bg-purple-600' : 'bg-gray-600'
                          }`}
                      >
                        <span
                          className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${voiceChatAutoPlay ? 'translate-x-5' : 'translate-x-1'
                            }`}
                        />
                      </button>
                    </label>
                  ...
                  <div className="space-y-2">
                    <label className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${isLiveVoiceMode ? 'bg-green-600' : 'bg-gray-600'
                          }`}
                      >
                        <span
                          className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${isLiveVoiceMode ? 'translate-x-5' : 'translate-x-1'
                            }`}
                        />
                      </button>
                    </label>
            ...
            <div className="px-4 py-3 flex-shrink-0 bg-transparent border-t border-white/6">
              ...
              {(isLiveVoiceMode || isRecording) && (
                <div className="px-2 py-1 mb-2 rounded-full bg-gray-800/80 border border-gray-600/50">
                  <div className="flex items-center space-x-2">
                    <div className={`w-2 h-2 rounded-full ${isLiveVoiceMode
              ...
              <div className="flex items-center space-x-2">
                ...
                {!audioBlob && !isRecording && (
                  <button
                    onClick={toggleVoiceMode}
                    className={`px-3 py-2 rounded-lg text-xs font-bold transition-all duration-200 ${isLiveVoiceMode
                ...
                {!audioBlob ? (
                  <button
                    ...
                    className={`p-2 rounded transition-colors ${isLiveVoiceMode
      ...
      {isVisible && !isFloatingMode && (
        <div
          ...
          style={{ width: 360, maxWidth: "92%", height: "60vh", maxHeight: "65vh" }}
        >
          <div className="chat-glass rounded-lg overflow-hidden shadow-lg h-full flex flex-col">
            {/* header */}
            <div className="flex items-center justify-between px-4 py-3 flex-shrink-0">
