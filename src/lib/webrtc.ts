...
class WebRTCManager {
  ...
  private fileAudioGain: GainNode | null = null
  private localPlaybackGain: GainNode | null = null
  private retryCounts: Map<string, number> = new Map()
  private liveVoiceStream: MediaStream | null = null
  private _isLiveVoiceModeActive = false
  ...
  private setupSocketListeners() {
    ...
    socketManager.onRoomInfo((room: any) => {
      if (!room) return
      if (this.localUserId && room.host && room.host.id === this.localUserId) {
        this.setHostStatus(true)
        const participantIds = (room.participants || []).map((p: any) => p.user?.id).filter((id: string) => id && id !== this.localUserId)
        this.ensureConnectionsTo(participantIds, this.localUserId)
      } else {
        this.setHostStatus(false)
        const hostId = room.host?.id
        if (hostId && hostId !== this.localUserId) {
          this.initializePeerConnection(hostId, false).catch(() => {})
        }
      }
    })
  ...
  async initializePeerConnection(peerId: string, isInitiator: boolean = false): Promise<RTCPeerConnection> {
    ...
    if (this.fileStream) {
      try { this.addOrReplaceTracks(pc, this.fileStream) } catch (e) {}
    }
    ...
    pc.onicecandidate = (event) => {
      if (event.candidate) socketManager.sendIceCandidate(event.candidate, peerId)
    }

    pc.ontrack = (event) => {
      this.handleTrackReceived(peerId, event)
    }
  ...
  private setupDataChannel(peerId: string, dataChannel: RTCDataChannel, isHost: boolean) {
    ...
    try { (dataChannel as any).bufferedAmountLowThreshold = 64 * 1024 } catch {}
    dataChannel.onopen = () => {
  ...
  public async startLiveVoiceChat(): Promise<MediaStream | null> {
    try {
      if (this.liveVoiceStream) this.stopLiveVoiceChat()
      ...
      this._isLiveVoiceModeActive = true
      return this.liveVoiceStream
    } catch (error) {
      this._isLiveVoiceModeActive = false
      return null
    }
  }
    ...
    if (this.liveVoiceStream) {
      ...
      this.peers.forEach(peer => {
        ...
        senders.forEach(sender => {
          if (sender.track && sender.track.kind === 'audio' && this.liveVoiceStream?.getAudioTracks().includes(sender.track)) {
  ...
  public isLiveVoiceActive() {
    return this._isLiveVoiceModeActive
  }
      ...
      if (audioCtx) {
        ...
        if (sourceNode) {
          ...
          for (let ch = 0; ch < MAX_CHANNELS; ch++) {
            ...
            try { splitter.connect(gL, ch) } catch (e) {}
            try { splitter.connect(gR, ch) } catch (e) {}
            try { gL.connect(merger, 0, 0) } catch (e) {}
            try { gR.connect(merger, 0, 1) } catch (e) {}
          }
